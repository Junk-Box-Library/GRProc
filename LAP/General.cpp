...
GENERAL_API MSGraph<sWord>* general_proc(MSGraph<sWord>* vp)
{
	...
	for (int i = 1; i < vp->xs - 1; i++) {
		for (int j = 1; j < vp->ys - 1; j++) {
			int a = vp->point(i - 1, j) + vp->point(i + 1, j) + vp->point(i, j - 1) + vp->point(i, j + 1);

			//int p = a - 4 * vp->point(i, j);

			int b = vp->point(i - 1, j - 1) + vp->point(i + 1, j - 1) + vp->point(i - 1, j - 1) + vp->point(i + 1, j + 1);
			int p = a + b - 8 * vp->point(i, j);
	
			//if (p < 0) p = -p;
			
			xp->point(i, j) = vp->point(i, j) - p/20;
		}
	}
